...
(function(root, factory) {
  ...
  ZaloPay.openCamera = function(opt) {
    if (!isObj(opt)) {
      writeLog('error', 'ZaloPay.openCamere', 'Received invalid object');
      return;
    }
  ...
  ZaloPay.openPhotoLibrary = function(opt) {
    if (!isObj(opt)) {
      writeLog('error', 'ZaloPay.openPhotoLibrary', 'Received invalid object');
      return;
    }
    // if (isStr(opt.desc) && isNumber(opt.needResult) && isArray(opt.scanType)) {
    const cb = opt.success;
    opt = {};
    ZaloPay.call('openPhotoLibrary', opt, cb);
    return;
  };
